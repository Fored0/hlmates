    ...
    <el-card class="card">
      <div class="current_address">
        ...
        <div class="address_container" v-if="currentAddress.length === 0">
          ...
          <div>详细地址:{{ renderData.receive[0].locationAddress + renderData.receive[0].detailAddress }}</div>
          <div>TEL:{{ renderData.receive[0].receivePhone }}</div>
        </div>
        <div class="address_container" v-else>
          <div>收货人:{{ currentAddress.receivePeople }}</div>
          <div>详细地址:{{ currentAddress.locationAddress + currentAddress.detailAddress }}</div>
          <div>TEL:{{ currentAddress.receivePhone }}</div>
        </div>
      </div>
      <div class="select_another">
        <el-button @click="selectDialogTableVisible = true">选择其他收货地址</el-button>
      </div>
      <div class="confirm_order">
        <p class="current_title">确认订单信息</p>
        <div>
          <el-table class="table_order" :data="tableData" border style="width: 100%">
            <el-table-column prop="imgUrl" label="图片" width="180"></el-table-column>
            <el-table-column prop="goodsName" label="商品，" width="180"></el-table-column>
            <el-table-column prop="count" label="姓名" width="180"></el-table-column>
            <el-table-column prop="payByMethods" label="姓名" width="180"></el-table-column>
            <el-table-column prop="price" label="地址"></el-table-column>
          </el-table>
        </div>
      </div>
    </el-card>
    <el-dialog title="选择其他收货地址" :visible.sync="selectDialogTableVisible">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="selectDialogTableVisible = false">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
